...
## 2024-10-27 - Missing Authentication on Administrative Endpoints
...
## 2024-10-27 - Directory Parameter Injection Allowing Access to System Directories
**Vulnerability:** The `GET /api/textures/:category` endpoint relied solely on path traversal checks (`path.relative`) to ensure the requested category was inside the `TEXTURES_DIR`. However, it failed to perform strict string validation to block access to legitimate internal system directories such as `Hidden` and `Uncategorized`. An attacker could simply supply `Hidden` as the category parameter to list all files inside this internal directory.
**Learning:** Preventing path traversal using `path.relative()` only restricts access to files outside the base directory. It does not prevent access to internal, sensitive, or system directories located within that base directory if explicit blacklisting or whitelisting is not implemented.
**Prevention:** Always combine path traversal protections with strict string validation (e.g., exact matches or explicit blacklists) to block access to internal directories that should not be exposed via public APIs.
