...
## 2026-03-21 - XSS Vulnerability in innerHTML
**Vulnerability:** The `viewer.js` frontend script constructed HTML strings and inserted them into the DOM using `innerHTML` with unsanitized variables like `mat.name`, `tex.name`, and `tex.url`. If these variables contain user-controlled data or data read from external files (like material names in a DAE file), it leads to DOM-based Cross-Site Scripting (XSS).
**Learning:** When generating HTML dynamically on the client-side using `innerHTML`, all variable data must be explicitly HTML-escaped. Setting attributes via `setAttribute` does not require HTML escaping since the browser treats the input as raw text.
**Prevention:** Always use a utility function like `escapeHtml` to sanitize variables before interpolating them into HTML strings intended for `innerHTML`, or prefer using `textContent` and `createElement` DOM APIs where possible.
