...
## 2024-11-20 - [Information Disclosure via Static Admin Directory]
**Vulnerability:** The `/admin` directory, which contains sensitive administration tools like `tagger.html`, was served publicly by the catch-all `express.static` middleware without any authentication. This happened because the `/admin` path itself lacked the `adminAuth` middleware protection, despite individual API endpoints being protected.
**Learning:** API route protection is not sufficient if the static frontend client used to interact with those APIs is served unauthenticated. An attacker could access the tool, understand its workings, and potentially find client-side vulnerabilities or craft unauthorized requests (even if the APIs themselves are protected, exposing the admin UI is a risk and leakage of sensitive workflow/information).
**Prevention:** Apply authentication middleware directly to the static route serving administrative or sensitive tools (e.g., `app.use('/admin', adminAuth, express.static(...))` or `app.use('/admin', adminAuth)`) BEFORE the general public static middleware.
