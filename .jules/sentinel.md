...
## $(date +%Y-%m-%d) - Secure Static Directories with Authentication
**Vulnerability:** Static administrative directories (`/admin`) and staging environments (`/showroom/staging`) were exposed to unauthenticated physical file access because explicit authentication middleware (`adminAuth`) was only applied to API routes, while general `express.static` middlewares served the files without protection or were shadowed by less specific routes.
**Learning:** API route protection does not automatically secure statically served physical assets in the same conceptual paths. Furthermore, when mounting overlapping static directories in Express (e.g., `/showroom` and `/showroom/staging`), the more specific route must be declared before the less specific catch-all route to prevent shadowing and ensure route-specific middleware executes correctly.
**Prevention:** Always mount explicit authentication middleware on routes serving sensitive physical files *before* any `express.static` catch-all middlewares. When dealing with nested static routes, declare the most specific routes first.
