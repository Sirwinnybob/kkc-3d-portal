...
## 2024-05-20 - Missing Authentication on Admin Static Route
**Vulnerability:** The application was serving the `/admin` static directory without authentication because the `adminAuth` middleware was not applied to that specific route, and instead was only applied to a few API endpoints. The static catch-all (`app.use(express.static('public'))`) allowed access to `/admin/tagger.html` without credentials.
**Learning:** This is a common pattern where static asset middleware shadows auth middleware or auth middleware is forgotten for static administrative tools. When securing static admin endpoints, the auth middleware must be explicitly mounted on the admin route path *before* any public static middleware.
**Prevention:** Always verify that route-specific middleware (`app.use('/admin', authMiddleware)`) is declared before general static file serving middlewares.
