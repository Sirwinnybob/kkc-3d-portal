...
## 2026-05-09 - [HIGH] Fix authorization bypass in admin endpoint
**Vulnerability:** Unauthenticated access to the `public/admin` directory allowed users to access sensitive admin tools (`tagger.html`).
**Learning:** `express.static('public')` serves any file in the public directory and its subdirectories. To protect specific subdirectories, an authentication middleware (like `adminAuth`) must be mounted at the specific route (`/admin`) *before* the general catch-all static handler.
**Prevention:** Ensure that path-specific access controls are always evaluated before falling through to generic file serving mechanisms.
