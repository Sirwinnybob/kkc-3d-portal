...
## 2025-06-05 - [Information Disclosure via Unauthenticated Admin Static Files]
**Vulnerability:** The `/admin` route lacked the `adminAuth` middleware and was served implicitly by the catch-all `express.static('public')` middleware. This allowed an unauthenticated attacker to access the static file `/admin/tagger.html` directly, even though its corresponding APIs were protected by `adminAuth`.
**Learning:** In Express, API protection does not cascade down to static files representing the UI of those APIs. To protect the static files (HTML, JS, CSS) corresponding to an administrative area, the static file route itself (or the specific path it serves from) must be explicitly protected by an authentication middleware before the catch-all public static middleware is evaluated.
**Prevention:** Always mount authentication middleware explicitly on protected subdirectories (like `/admin`) before using a generic `express.static` that might inadvertently serve those files. E.g. `app.use('/admin', adminAuth);`.
