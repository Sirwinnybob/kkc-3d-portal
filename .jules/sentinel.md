...
## 2024-11-20 - [Information Disclosure via Admin Bypass]
**Vulnerability:** The `/admin` route which is used to serve admin files and static assets was completely unprotected and available to the public. There was an `adminAuth` middleware implemented but it wasn't used for the `/admin` static route. Also, `/showroom/staging` which is a private static directory was unprotected.
**Learning:** Security middlewares need to be actively applied to the routes they are meant to protect. Creating a middleware does not secure the app; it must be connected to the express pipeline. Serving static files should also be scrutinized as heavily as API endpoints to ensure no sensitive files are accidentally leaked.
**Prevention:** Apply the correct authentication middlewares to all sensitive routes, including static directories (`app.use('/admin', adminAuth)` and `app.use('/showroom/staging', adminAuth, express.static(...))`).
