...
## 2024-05-24 - Path Traversal via Absolute Paths in path.resolve
**Vulnerability:** Using `path.resolve(BASE_DIR, user_input)` is vulnerable if `user_input` is an absolute path (e.g., `/etc/passwd`). Node's `path.resolve` will ignore `BASE_DIR` if a subsequent argument is absolute.
**Learning:** Even with a `startsWith` check, `path.resolve` can be bypassed if the input is treated as an absolute path by the OS.
**Prevention:** Ensure user input is treated as a relative path component by using `path.join('.', user_input)` before passing it to `path.resolve`, then verify the result with `startsWith(BASE_DIR + path.sep)`.
