...
## 2024-06-05 - [Information Disclosure via Overlapping Express Static Routes]
**Vulnerability:** The `/showroom/staging` and `/admin` directories were not fully protected. Even though they had API protections or were mounted to `express.static` with `adminAuth`, the more general catch-all static route `express.static('public')` or `express.static(SHOWROOM_DIR)` would match the requested route and serve the file bypassing authentication, due to Express evaluating middleware sequentially.
**Learning:** When mounting overlapping static directories in Express (e.g., `/showroom` and `/showroom/staging`), the more specific route must be declared before the less specific catch-all route to prevent shadowing and ensure route-specific middleware is correctly executed. Additionally, protecting static files like `/admin/*` requires explicit middleware matching mounted *before* general static handlers.
**Prevention:** Always mount secure static file routes using authentication middleware before any generic/catch-all static route declarations that cover overlapping file system paths or logical route prefixes.
