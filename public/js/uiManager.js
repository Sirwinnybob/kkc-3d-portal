export class UIManager {
    ...
    init() {
        ...
        window.addEventListener('keydown', (e) => {
            ...
            if (key === 'm') { e.preventDefault(); document.getElementById('menu-btn')?.click(); }
            if (key === 't') { e.preventDefault(); document.getElementById('texture-btn')?.click(); }
            if (key === 'c') { e.preventDefault(); document.getElementById('camera-btn')?.click(); }
            ...
            if (e.key === '+' || e.key === '=') {
                document.getElementById('joystick-handle')?.dispatchEvent(new KeyboardEvent('keydown', { key: 'PageUp', bubbles: true }));
            }
            if (e.key === '-' || e.key === '_') {
                document.getElementById('joystick-handle')?.dispatchEvent(new KeyboardEvent('keydown', { key: 'PageDown', bubbles: true }));
            }
        });

        window.addEventListener('keyup', (e) => {
            if (['+', '=', '-', '_'].includes(e.key)) {
                document.getElementById('joystick-handle')?.dispatchEvent(new KeyboardEvent('keyup', { bubbles: true }));
            }
        });
    }
        ...
        if (lightModeBtn) {
            const updateLightModeUI = () => {
                ...
                if (isLightMode) {
                    ...
                    lightModeBtn.title = 'Switch to Dark Mode (L)';
                    lightModeBtn.setAttribute('aria-label', 'Switch to Dark Mode');
                    ...
                    lightModeBtn.title = 'Switch to Light Mode (L)';
                    lightModeBtn.setAttribute('aria-label', 'Switch to Light Mode');
                    lightModeBtn.innerHTML = `
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
